...
Mathematically, a 'basis' is an orthonormal set of `d` element vectors, where each element vector is an linear combination of `p` elements in vector-space.  Esentially a basis defines the direction and magnitude each of the `p` attributes contributes to every direction of the `d`-dimensional projection space. Consider a projection down to 2D. The basis defining this projection can be visually displayed as a unit circle with `p` line segments steming from the origin as shown in figure \@ref(fig:basis).
...
There are many methods that idenitfy one or more projections of interest. For instance scatterplot matrices [@chambers_graphical_1983] can be used to rapidly valid univariate supports and distributions. Principal component analysism [PCA; @pearson_liii._1901]  with the use of biplots highlight full sample variation. Linear discriminant analysis [@fisher_use_1936] and penalized discriminant analysis [@hastie_penalized_1995] can be used to idenfify projection with cluster separation. These are great techniques and all applicable for numeric data with many attributes, however, they have two notable features: they show only 1 or several _discrete_ projections and they do not allow for an analyst to steer or control.
...
Current practice for high-dimensional data is to make a single low-dimensional representation, with a method such as principal component analysis (PCA). Often just the first two principal components are shown, which is equivalent to a 2D projection. This technique is also called a biplot. Another more rarely used technique is a tour, which shows an animation of 2D projections. It can be used to get an overview of the structure in the multivariate space, but it is difficult to assess the importance of variables to particularly interesting low-dimensional structure. A human subject study has be designed and conducted to assess the learning about importance of variables, from each of these techniques. 
...
The cutting edge of visualisation research today is in virtual environments, as technology has advanced to make this accessible to the masses. It is interesting to explore how analysis with 3D graphics operates in comparison with 2D graphics. Building a geodesic interpolator for 3D projections extends the technology from 2D to 3D, and will allow us to compare the benefits or not, of the new environment for analysing with multivariate data.
