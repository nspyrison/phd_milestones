...
1. **How do we define a geodesic interpolator be used to add and remove variables smoothly from a 2D linear projection of data? <!--What is the ideal form of an interoperable application of manual tour?-->**\
...
<!--to should be better supported, especially proof for the rotation matrix identified. An application of this ideally embodies two properties: interoperable with other tour applications and allow for 'next step' interaction for UCS. Manual tours, and specifically the fine control of UCS, should allow analysts to better explore the sensitivity of structure from single variable contributions on the projection. This also acts as the foundation to be used in the remaining objectives.-->

2. **Do analysts understand the relationship between variables and structure in a 2D linear projection better when the geodesic interpolator is available? <!--2D interactive manual tours provide benefits over alternatives?-->**\
Current practice for high-dimensional data is to make a single low-dimensional representation, with a method such as principal component analysis (PCA). Often just the first two principal components are shown, which is equivalent to a 2D projection. This technique is also called a biplot. Another more rarely used technique is a tour, which shows a movie of 2D projections. It can be used to get an overview of the structure in the multivariate space, but it is difficult to assess the importance of variables to particularly interesting low-dimensional structure. A human subjects study will be designed and conducted to assess the learning about importance of variables, from each of these techniques. 
