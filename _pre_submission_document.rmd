...
The research for RO #3 involves _expert review_ of visualization design. We know that the SAHP value is a local explanation for one observation. This SHAP value will also serve as the 1D basis for the manual tour. While using SHAP as a projection basis is novel it is not particularly insightful by itself. We provide tracking marks on the tour as well as showing the within-class distributions of the SHAP components as parallel coordinate marks on the basis. We also offer a global view and quantitative analysis evaluating the sensitivity of the SHAP space relative to the sensitivity of the original data space.
...
## created in latex above.
knitr::include_graphics("figures/spinifex_study_y1_results.PNG")
```

A more in-depth description and discussion of this user study is attached as appendix A, a draft version of the paper we intend to submit to the Journal of Data Science, Statistics, and Visualization. There we also use the the same mixed regression model to predict log response time where grand tour has the fastest responses, presumably due to the lack of interaction.
...
We approximate the data and SHAP spaces as the first 2 principal components. With interactive application, we further couple this display with linked brushing and display the tabular subset of the selected observations.
