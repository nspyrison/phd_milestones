...
In summary, we use a mixed regression model, using the factors above as main effects, and use the participant and data simulations as random effects. The model regressing on the accuracy. We select on the following model.
...
Given the global view above we want to look at the local weightings of primary and comparison points (shown as '*/x' above and dashed/dotted lines below). In this case, the primary observation is a misclassified observation while the comparison point is a correctly classified nearby point. The idea is to view how sensitive SHAP values are to this misclassified points. Two points that are classified differently, but otherwise have very similar values in the explanatory variables, often lie quite far apart in SHAP space.

Now we have the global context, and an idea of the sensitivity of the SHAP spaces. We can further explore an agnostic local explanation by exploring the structure identified by the SHAP values of a particular observation. Consider the selected observation labeled with as a star/atersk. Its normalized SHAP value will become the starting 1D basis to perform a radial tour exploring the structure, in this case separation from other clusters.
